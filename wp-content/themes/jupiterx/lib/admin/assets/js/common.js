...
(function ($, wp) {
  ...
  function installPro() {
    function installNow() {
      ...
      $template.prepend(wp.template('jupiterx-progress-bar')());
  ...
  function updatePluginsNotce() {
    var $hidden = $('.jupiterx-update-plugins-notice input[type=hidden]');

    if (!$hidden.length) {
      return;
    }

    var plugins = JSON.parse($hidden.val());

    if (!plugins.length) {
      return;
    }

    var updates = plugins.filter(function (plugin) {
      return plugin['action'] === 'update';
    });

    var installs = plugins.filter(function (plugin) {
  ...
  window.jupiterx = jQuery.extend({}, window.jupiterx, {
    ...
    updatePluginsNotce: updatePluginsNotce,
    updatePluginsModal: updatePluginsModal
  });
