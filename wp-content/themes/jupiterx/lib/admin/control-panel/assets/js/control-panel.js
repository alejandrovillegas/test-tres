...
(function ($) {
   ...
   *******************************************************/
		...
		var plugins = {
				...
				init: function init() {
						...
						$(document).on('click', '.abb_plugin_activate', plugins.activateInit);
						$(document).on('click', '.abb_plugin_deactivate', plugins.deactivateInit);
						$(document).on('click', '.abb_plugin_install', plugins.installInit);
						$(document).on('click', '.abb_plugin_delete', plugins.deleteInit);
						$(document).on('click', '.abb_plugin_update', plugins.updateInit);
						$(document).on('click', '.jupiterx-cp-plugins-filter > .btn', plugins.filtersInit);
				},

				getPluginsList: function getPluginsList() {
						var data = { action: 'abb_get_plugins' };

						$.post(ajaxurl, data, function (response) {
								$('#js__jupiterx-plugins').html('');
								if (response.status == true) {
										if (response.data.hasOwnProperty('limit')) {
												$('#js__jupiterx-plugins').attr('data-plugins-limit', response.data.limit);
										}
										$.each(response.data.plugins, function (key, val) {
												$('#js__jupiterx-plugins').append(plugins.getPluginsTemplate(val));
										});
				...
				getPluginsTemplate: function getPluginsTemplate(pluginData) {
						var btns = this.getPluginActionBtns(pluginData),
						    dataFilter = pluginData.active == true ? 'active' : 'inactive',
						    dataUpdate = pluginData.update_needed == true && pluginData.installed ? 'yes' : 'no',
						    cloudClass = pluginData.installed ? '' : 'jupiterx-icon-cloud';

						var template = '<div class="jupiterx-cp-plugin-item" data-filter="' + dataFilter + '" data-update="' + dataUpdate + '">' + '<div class="jupiterx-cp-plugin-item-inner jupiterx-card">' + (pluginData.pro ? '<img class="jupiterx-pro-badge" src="' + jupiterxControlPanel.proBadgeUrl + '" />' : '') + '<div class="jupiterx-card-body">' + '<figure class="jupiterx-cp-plugin-item-thumb ' + cloudClass + '">' + '<img src="' + pluginData.img_url + '">' + '</figure>' + '<div class="jupiterx-cp-plugin-meta-wrapper">' + '<span class="jupiterx-cp-plugin-item-required is-active-' + pluginData.required + '">' + jupiterx_cp_textdomain.required + '</span>' + '<span class="jupiterx-cp-plugin-item-version">v<span class="item-version-tag">' + pluginData.version + '</span></span>' + '</div>' + '<div class="jupiterx-cp-plugin-item-meta">' + '<div class="jupiterx-cp-plugin-item-name">' + pluginData.name + '</div>' + '<div class="jupiterx-cp-plugin-item-desc">' + pluginData.desc + (pluginData.more_link ? ' <a href="' + pluginData.more_link + '" target="_blank">Learn More</a>' : '') + '</div>' + '</div>' + btns + '</div>' + '</div>' + '</div>';
						return template;
				},

				getPluginActionBtns: function getPluginActionBtns(pluginData) {

						var is_multisite = document.body.classList.contains('multisite');

						if (!pluginData.installed) {
								if (pluginData.pro) {
										return this.getProBtn();
								}
								var classes = pluginData.install_disabled ? 'disabled' : '';
								return this.getInstallBtn(pluginData.slug, pluginData.name, pluginData.basename, classes);
						}

						if (pluginData.network_active) {
								return this.getNetworkActiveLabel();
						}

						var btns = '';

						if (pluginData.active == true) {
								btns += this.getDeactivateBtn(pluginData.slug, pluginData.name, pluginData.basename);
						} else {
								btns += this.getActivateBtn(pluginData.slug, pluginData.name, pluginData.basename);
								if (!is_multisite) {
										btns += this.getDeleteBtn(pluginData.slug, pluginData.name, pluginData.basename);
								}
						}

						if (pluginData.update_needed == true && !is_multisite) {
								btns += this.getUpdateBtn(pluginData.slug, pluginData.name, pluginData.basename);
						}

						return btns;
				},

				getProBtn: function getProBtn() {
						return '<a class="btn btn-sm btn-outline-success jupiterx-upgrade-modal-trigger" data-upgrade-link="https://themeforest.net/item/jupiter-multipurpose-responsive-theme/5177775?ref=artbees&utm_source=PluginSectionImportButton&utm_medium=AdminUpgradePopup&utm_campaign=FreeJupiterXAdminUpgradeCampaign" href="#">' + jupiterx_cp_textdomain.add + '</a>';
				},

				getNetworkActiveLabel: function getNetworkActiveLabel() {
						return '<span class="btn btn-sm network-active" href="#">' + jupiterx_cp_textdomain.network_active + '</span>';
				},

				getInstallBtn: function getInstallBtn(slug, name, basename, classes) {
						return '<a class="btn btn-sm btn-outline-success jupiterx-icon-plus-circle-solid abb_plugin_install ' + classes + '" data-basename="' + basename + '" data-slug="' + slug + '" href="#" data-name="' + name + '">' + jupiterx_cp_textdomain.add + '</a>';
				},

				getUpdateBtn: function getUpdateBtn(slug, name, basename) {
						return '<a class="btn btn-sm btn-outline-primary jupiterx-icon-sync-alt abb_plugin_update" data-basename="' + basename + '" data-slug="' + slug + '" href="#" data-name="' + name + '">' + jupiterx_cp_textdomain.update + '</a>';
				},

				getDeleteBtn: function getDeleteBtn(slug, name, basename) {
						return '<a class="btn btn-sm btn-outline-danger jupiterx-icon-times-circle abb_plugin_delete" data-basename="' + basename + '" data-slug="' + slug + '" href="#" data-name="' + name + '">' + jupiterx_cp_textdomain.delete + '</a>';
				},

				getActivateBtn: function getActivateBtn(slug, name, basename) {
						var limit = plugins.getActivationLimit(slug);
						return '<a class="btn btn-sm btn-primary abb_plugin_activate" data-basename="' + basename + '" data-slug="' + slug + '" href="" data-name="' + name + '" data-limit="' + limit + '">' + jupiterx_cp_textdomain.activate + '</a>';
				},

				getDeactivateBtn: function getDeactivateBtn(slug, name, basename) {
						return '<a class="btn btn-sm btn-danger abb_plugin_deactivate" data-basename="' + basename + '" data-slug="' + slug + '" href="#" data-name="' + name + '">' + jupiterx_cp_textdomain.deactivate + '</a>';
				},

				getActivationLimit: function getActivationLimit(slug) {
				...
				installInit: function installInit(e) {
						e.preventDefault();
						var $this = $(this),
						...
						jupiterx_modal({
								...
								text: plugins.language(jupiterx_cp_textdomain.you_are_about_to_install, [pluginName]),
								type: 'warning',
								showCancelButton: true,
								showConfirmButton: true,
								...
								showProgress: false,
								onConfirm: function onConfirm() {
										plugins.installStart($this.data('slug'));
								}
						});
				},

				installStart: function installStart(pluginSlug) {
						var self = this,
						    $btn = $('.abb_plugin_install[data-slug="' + pluginSlug + '"]'),
						    pluginName = $btn.data('name'),
						    pluginBasename = $btn.data('basename'),
						    data = {
						...
						$.post({
								...
								success: function success(response) {
										if (response.hasOwnProperty('status')) {
												if (response.status == true) {
														$btn.replaceWith(self.getActivateBtn(pluginSlug, pluginName, pluginBasename) + self.getDeleteBtn(pluginSlug, pluginName, pluginBasename));

														jupiterx_modal({
																title: jupiterx_cp_textdomain.plugin_is_successfully_installed,
																text: plugins.language(jupiterx_cp_textdomain.plugin_installed_successfully_message, [pluginName]),
																type: 'success',
																showCancelButton: false,
																showConfirmButton: true,
																showCloseButton: false,
				...
				activateInit: function activateInit(e) {
						e.preventDefault();
						var $this = $(this);

						jupiterx_modal({
								title: jupiterx_cp_textdomain.activating_notice,
								text: plugins.language(jupiterx_cp_textdomain.are_you_sure_you_want_to_activate, [$this.data('name')]),
								type: 'warning',
								showCancelButton: true,
								showConfirmButton: true,
								confirmButtonText: jupiterx_cp_textdomain.continue,
				...
				activate_start: function activate_start(pluginSlug) {
						var self = this,
						    $btn = $('.abb_plugin_activate[data-slug="' + pluginSlug + '"]'),
						    pluginName = $btn.data('name'),
						    pluginBasename = $btn.data('basename'),
						    data = {
								action: 'abb_activate_plugin',
								slug: pluginSlug,
								basename: pluginBasename
						};
								...
								success: function success(response) {
										if (response.hasOwnProperty('status')) {
												if (response.status == true) {
														...
														$btn.replaceWith(self.getDeactivateBtn(pluginSlug, pluginName, pluginBasename));
														jupiterx_modal({
																title: jupiterx_cp_textdomain.all_done,
																text: plugins.language(jupiterx_cp_textdomain.item_is_successfully_activated, [pluginName]),
																type: 'success',
																showCancelButton: false,
																showConfirmButton: true,
																showCloseButton: false,
				...
				deactivateInit: function deactivateInit(e) {
						e.preventDefault();
						var $this = $(this);

						jupiterx_modal({
								title: jupiterx_cp_textdomain.important_notice,
								text: plugins.language(jupiterx_cp_textdomain.are_you_sure_you_want_to_deactivate, [$this.data('name')]),
								type: 'warning',
								showCancelButton: true,
								showConfirmButton: true,
								confirmButtonText: jupiterx_cp_textdomain.continue,
				...
				deactivate_start: function deactivate_start(pluginSlug) {
						var self = this,
						    $btn = $('.abb_plugin_deactivate[data-slug="' + pluginSlug + '"]'),
						    pluginName = $btn.data('name'),
						    pluginBasename = $btn.data('basename'),
						    is_multisite = document.body.classList.contains('multisite'),
						    data = {
								...
								success: function success(response) {
										if (response.hasOwnProperty('status')) {
												if (response.status == true) {
														...
														var newBtns = self.getActivateBtn(pluginSlug, pluginName, pluginBasename);

														if (!is_multisite) {
																newBtns += self.getDeleteBtn(pluginSlug, pluginName, pluginBasename);
														}

														$btn.replaceWith(newBtns);
				...
				deleteInit: function deleteInit(e) {
						e.preventDefault();
						var $this = $(this);

						jupiterx_modal({
								title: jupiterx_cp_textdomain.delete_plugin,
								text: plugins.language(jupiterx_cp_textdomain.you_are_about_to_delete, [$this.data('name')]),
								type: 'warning',
								showCancelButton: true,
								showConfirmButton: true,
								confirmButtonText: jupiterx_cp_textdomain.continue,
								...
								showProgress: false,
								onConfirm: function onConfirm() {
										plugins.delete_start($this.data('slug'));
								}
						});
				},

				delete_start: function delete_start(pluginSlug) {
						...
						wp.updates.ajax('delete-plugin', {
								...
								success: function success(response) {
										jupiterx_modal({
												...
												text: plugins.language(jupiterx_cp_textdomain.plugin_deleted_successfully_message, [pluginName]),
												type: 'success',
												showCancelButton: false,
												showConfirmButton: true,
												showCloseButton: false,
										...
										$btn.siblings('.btn').remove() - $btn.replaceWith(plugins.getInstallBtn(pluginSlug, pluginName, pluginBasename));
								},
				...
				updateInit: function updateInit(e) {
						e.preventDefault();
						var $this = $(this),
						    pluginName = $this.data('name');
						...
						jupiterx_modal({
								...
								text: plugins.language(jupiterx_cp_textdomain.you_are_about_to_update, [pluginName]),
								type: 'warning',
								showCancelButton: true,
								showConfirmButton: true,
								confirmButtonText: jupiterx_cp_textdomain.continue,
								...
								showProgress: false,
								onConfirm: function onConfirm() {
										plugins.update_start($this.data('slug'));
								}
						});
				},

				update_start: function update_start(pluginSlug) {
						...
						wp.updates.ajax('update-plugin', {
								...
								success: function success(response) {
										jupiterx_modal({
												...
												text: plugins.language(jupiterx_cp_textdomain.plugin_updated_recent_version, [$btn.data('name')]),
												type: 'success',
												showCancelButton: false,
												showConfirmButton: true,
												showCloseButton: false,
  ...
  *******************************************************/
		...
		var templates = {
				...
				install_types: ['preparation', 'backup_db', 'backup_media_records', 'reset_db', 'upload', 'unzip', 'validate', 'plugin', 'theme_content', 'setup_pages', 'plugins_content', 'settings', 'menu_locations', 'theme_widget', 'restore_media_records', 'finalize'],
				template_id: null,
				template_name: null,
				import_media: false,
				progress_bar_html: '',
				...
				get_template_list_html: function get_template_list_html(data) {
						...
						if (installed == false) {
								var importBtnAttrs = data.free_template === '1' ? 'class="btn btn-primary mr-2 jupiterx-cp-template-item-btn js__cp_template_install" href="#" data-title="' + template_name.replace(' Jupiterx', '') + '" data-name="' + data.name + '" data-slug="' + data.slug + '" data-id="' + data.id + '"' : 'class="btn btn-primary mr-2 jupiterx-cp-template-item-btn jupiterx-upgrade-modal-trigger" href="#" data-upgrade-link="https://themeforest.net/item/jupiter-multipurpose-responsive-theme/5177775?ref=artbees&utm_source=TemplateSectionImportButton&utm_medium=AdminUpgradePopup&utm_campaign=FreeJupiterXAdminUpgradeCampaign"';
								var btn = '<a ' + importBtnAttrs + '>' + jupiterx_cp_textdomain.import + '</a>' + '<a class="btn btn-outline-secondary mr-2 jupiterx-cp-template-item-btn" href="http://demos.artbees.net/jupiterx/' + template_slug.replace('-jupiterx', '') + '" target="_blank">' + jupiterx_cp_textdomain.preview + '</a>';
						} else {
						...
						return '<div class="jupiterx-cp-template-item">' + '<div class="jupiterx-cp-template-item-inner jupiterx-card">' + (data.free_template !== '1' ? '<img class="jupiterx-pro-badge" src="' + jupiterxControlPanel.proBadgeUrl + '" />' : '') + '<figure class="jupiterx-cp-template-item-fig">' + '<img src="' + data.img_url + '" alt="' + data.name + '">' + '</figure>' + '<div class="jupiterx-cp-template-item-meta jupiterx-card-body">' + '<h4 class="jupiterx-cp-template-item-name">' + template_name.replace(' Jupiterx', '') + '</h4>' + '<div class="jupiterx-cp-template-item-buttons' + (data.psd_file ? ' has-psd' : '') + '">' + btn + '</div>' + '</div>' + '</div>' + '</div>';
				},
						...
						jupiterx_modal({
								...
								onConfirm: function onConfirm() {
										jupiterx_modal({
												...
												text: templates.language(jupiterx_cp_textdomain.would_you_like_to_import_images_and_videos_as_preview, ['<a href="https://help.artbees.net/getting-started/installing-a-template" target="_blank">Learn More</a>']),
												type: 'warning',
												showCancelButton: true,
												showConfirmButton: true,
				...
				request_error_handling: function request_error_handling(XMLHttpRequest, textStatus, errorThrown) {
						...
						if (XMLHttpRequest.readyState == 4) {
								...
								if (XMLHttpRequest.responseText != '') {
										...
										custom_html += '</div>';
										custom_html += '<div class="jupiterx-modal-desc">' + templates.language(jupiterx_cp_textdomain.dont_panic, [XMLHttpRequest.status, '<a href="https://help.artbees.net/getting-started/installing-a-template" target="_blank">Learn More</a>']) + '</div>';
										...
										jupiterx_modal({
												title: jupiterx_cp_textdomain.something_went_wrong,
												text: templates.language(jupiterx_cp_textdomain.dont_panic, [XMLHttpRequest.status, '<a href="https://help.artbees.net/getting-started/installing-a-template" target="_blank">Learn More</a>']),
												type: 'error',
												showCancelButton: false,
												showConfirmButton: true,
												showLearnmoreButton: false
  ...
  *******************************************************/
   ...
   *******************************************************/
		function jupiterx_get_system_report() {
				...
				$('[data-jupiterx-ajax]').each(function () {
						var $self = $(this);
						var type = $self.data('jupiterxAjax');
						var feedbackIcon = $self.find('.status-state');
						var feedbackText = $self.find('.status-text');
   ...
   */
		...
		// Initialize pro badge tooltip.
		function initProBadgeTooltip() {
				$('.jupiterx-pro-badge').tooltip({
						title: jupiterx_cp_textdomain.pro_badge_tooltip_title,
						trigger: 'hover',
						container: '.jupiterx-wrap',
						template: '<div class="tooltip jupiterx-pro-badge-tooltip" role="tooltip"><div class="arrow"></div><div class="tooltip-inner"></div></div>'
				});
		}
})(jQuery);