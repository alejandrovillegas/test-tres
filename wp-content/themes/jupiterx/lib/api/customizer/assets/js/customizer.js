...
(function ($, wp, jupiterx) {
    ...
    'jupiterx-template': {
      ...
      ready: function ready() {
        ...
        control.elements = {
          ...
          control.elements.$control.toggleClass('jupiterx-has-value', hasValue);
        });
        ...
        $edit.on('click', function (event) {
          ...
          jupiterx.elementor.openEditor({
            url: control.getEditTemplateUrl(),
        ...
        $add.on('click', function (event) {
          ...
          jupiterx.elementor.openEditor({
            url: control.getNewTemplateUrl(),
            beforeClose: function beforeClose(contentWindow) {
              control.setting.set(contentWindow.elementor.config.post_id);
              $select.val(contentWindow.elementor.config.post_id);
              control.updateSelectTemplates();
            }
          });
        });

        // Update options inside select field.
        control.updateSelectTemplates();
      },

      /**
       * Update select options via AJAX.
       *
       * @since 1.0.0
       *
       * @returns {void}
       */
      updateSelectTemplates: function updateSelectTemplates() {
        var control = this,
            params = control.params,
            $control = control.elements.$control,
            $select = control.elements.$select,
            templateId = control.setting.get();

        jupiterx.elementor.getTemplates({
          data: {
            type: control.params.templateType
          },
          ...
          success: function success(templates) {
            ...
            $control.toggleClass('jupiterx-has-value', templateId ? true : false);
      ...
      /**
       * Get complete edit template url.
       *
       * @since 1.0.0
       *
       * @returns {string}
       */
      getEditTemplateUrl: function getEditTemplateUrl() {
        var params = this.params;
        return params.editTemplateUrl + '&post=' + this.setting.get();
      },

      /**
       * Get complete new template url.
       *
       * @since 1.0.0
       *
       * @returns {string}
       */
      getNewTemplateUrl: function getNewTemplateUrl() {
        var params = this.params;
        return params.newTemplateUrl + '&template_type=' + params.templateType;
      }
    }
   ...
   * @constructor
   */
  JupiterX.Template = JupiterX.Control.extend(getUniqueComponents('jupiterx-template'));
